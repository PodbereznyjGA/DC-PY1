...
def to_csv_file(filename: str, headers: [str], rows: list[list],
    ...
    :param rows: список со значениями
    :param delimiter: знак между значениями
    :param new_line: переход на новую строку
    :return:
    """
    with open(OUTPUT_FILE, 'w') as csv_file:
        csv_file.write(delimiter.join(headers) + new_line)
        for elem in rows:
            csv_file.write(delimiter.join(elem) + new_line)
